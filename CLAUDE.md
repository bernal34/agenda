...
- **Tema claro/oscuro**: la app sigue al sistema, sin interruptor. `ThemeProvider` (`lib/theme.tsx`) va en `app/_layout.tsx` y expone `useTheme()` (devuelve `t` y `scheme`) y `useThemedStyles(factory)`. En componentes nuevos: `const makeStyles = (t: Tokens) => StyleSheet.create({…})` a nivel de módulo y `const styles = useThemedStyles(makeStyles)` adentro; para colores que no van en la hoja (íconos, props de librerías) usá `const { t } = useTheme()`. **No importes `tokens` dentro de un componente**: se evalúa al cargar el módulo y congela el esquema claro. `tokens` sigue exportado como el juego claro para las pantallas todavía sin convertir. Convertidos: las 5 pestañas, la tab bar, `components/ui/*` y `TaskCard`; el resto (kanban, detalle de tarea, chat de canal, admin, config por área, auth) sigue en claro fijo.
...
- **Fechas**: `DateField` (`label`, `value` en `DD/MM/YYYY`, `onChange`) abre un calendario propio en las dos plataformas, armado con `buildMonthCells` de `lib/calendarGrid.ts`. No uses `Input` de texto para fechas ni `<input type="date">`: eso dejaba el celular sin calendario y metía colores hardcodeados en web.
- **Rutas modales**: envolvé la pantalla en `ModalScreen` (`onClose`, `maxWidth`). En web dibuja una ventana flotante centrada con backdrop que cierra al tocar fuera; en nativo no envuelve nada, porque ahí el modal a pantalla completa es lo idiomático. La ruta se declara en `app/(app)/_layout.tsx`, que ya usa `transparentModal` en web para que se vea la app detrás.
