...
function mentionToken(name: string): string {
  return name
    .normalize('NFD')
    // `\p{M}` = marcas combinantes (los acentos que deja NFD). Se usa la
    // categoría Unicode en vez del rango ̀-ͯ para que el fuente
    // quede en ASCII y no dependa de caracteres invisibles.
    .replace(/\p{M}/gu, '')
    .replace(/[^\w.-]+/g, '_')
    .replace(/^_+|_+$/g, '');
}
...
// Recibe la hoja de estilos: el color de la mención depende del tema y esta
// función vive fuera del componente.
...
export function TaskComments({ taskId, areaId, userId }: Props) {
  ...
  const handleSend = async () => {
    ...
    const matched = Array.from(new Set(text.match(MENTION_TOKEN) ?? []))
      .map((tok) => handleMap.get(tok.slice(1).toLowerCase()))
      .filter((id): id is string => !!id && id !== userId);
    setDraft('');
    try {
      await createMut.mutateAsync({ body: text, mentions: matched });
    } catch (err) {
      setDraft(text);
  ...
  return (
    <View style={styles.section}>
      ...
      {isLoading && <ActivityIndicator color={t.brand[600]} style={{ marginTop: 8 }} />}
      {error && (
      ...
      <View style={styles.composer}>
        ...
        >
          {createMut.isPending ? (
            ...
            <Send size={14} color={t.brand.fg} strokeWidth={2.2} />
          )}
        </Pressable>
      </View>
    </View>
  ...
  return (
    <View style={styles.commentRow}>
      ...
      <View style={{ flex: 1 }}>
        ...
        <Text style={styles.body}>{renderBody(comment.body, styles)}</Text>
      </View>
    </View>
  );
}
...
const makeStyles = (t: Tokens) => StyleSheet.create({
  ...
  suggestionPressed: { backgroundColor: t.brand[50] },
  suggestionName: {
    flex: 1,
    color: t.text.primary,
    fontSize: typography.size.sm,
  ...
  sendDisabled: { backgroundColor: t.brand[100] },
  sendPressed: { backgroundColor: t.brand[700] },
